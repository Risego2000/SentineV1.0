...
export class ForensicQueueV3 {
  ...
  enqueue(
    ...
    supabase
      .from(SUPABASE_TABLES.AUDIT_JOBS)
      .insert({
        id: job.id,
        status: job.status,
        directives: job.directives,
        audit_preset: job.auditPreset,
        rule_id: job.ruleId,
        viewer_id: job.viewerId,
        track_state: job.trackState,
        geometry_state: job.geometryState,
        snapshot: job.snapshot,
      })
      .then(({ error }) => {
        if (error) console.error('[ForensicQueue] Supabase job sync error:', error);
      });
  ...
  private async processNext() {
    ...
    try {
      ...
      supabase
        .from(SUPABASE_TABLES.AUDIT_JOBS)
        .update({ status: 'processing', status_changed_at: new Date().toISOString() })
      ...
      if (isRecoverableAIError) {
        ...
        if (!requeued) {
          ...
          supabase
            ...
            .update({
              status: 'failed',
              error_message: `Fallo tras reintentos: ${errorMessage}`,
              status_changed_at: new Date().toISOString(),
            })
            .eq('id', current.job.id)
            .then();
