...
const isVisionCapableModel = (modelName) => {
  const model = modelName.toLowerCase();
  return VISION_CAPABLE_MODELS.some((m) => model.includes(m)) && !model.includes('lite');
};
...
export const generateGeometryWithGemini = async ({ directives, instruction, image }) => {
  ...
  const prompt = `DIRECTIVAS DE INFRACCIÓN: "${directives}". PETICIÓN ADICIONAL: "${
    ...
    1. Crear una GEOMETRÍA DE DETECCIÓN precisa basada en las directivas proporcionadas.
    ...
    - "type": 'forbidden', 'stop_line', 'lane_divider', 'box_junction', 'pedestrian', 'bus_lane', 'roi_general', 'roi_turn'.
...
export const analyzeTrajectoryWithGemini = async ({
  ...
  const promptText = `
    ...
    ANÁLISIS REQUERIDO:
    1. Analiza los puntos de la trayectoria proporcionados.
    ...
    OUTPUT JSON (Estricto):
    {
      ...
      "plate": "Matrícula leída de la imagen o DESCONOCIDO si no es visible",
      "makeModel": "Identificación visual del vehículo",
