...
- ✅ Queries por estado, búsqueda por placa
...
- ✅ Interfaz responsiva
- ✅ Inline styling completo (sin dependencias de CSS externo)

#### 3️⃣ **Exportación PDF**
```
PDFExportService
├── Verifica firma digital (SHA-256)
├── Genera HTML con styling
...
USUARIO ABRE APP
    ↓
App.tsx (view toggle: detection ↔ expedients)
    ↓
    ├─→ [DETECTION MODE] MultiViewerGrid + SharedBottomBar
    │
    └─→ [EXPEDIENTS MODE]
        ↓
        ExpedientListPage
        ...
        │   ↓ (datos retornan)
        │   Mapeo: DB row → Typescript object
        │
        └─→ UI muestra lista
            ...
            └── Callback onStateChange() actualiza UI
...
2. `App.tsx` - View toggle y navigation
...
ExpedientRepository.getByState()
  ↓
Supabase.from('expedients').select().eq('state', state)
  ↓ (SELECT SQL se ejecuta)
Datos retornan, se mapean, UI se actualiza
```

### Transiciones de Estado
```
Usuario clicks "Validar"
  ↓
ExpedientWorkflow.handleValidate()
  ↓
ExpedientService.validateExpedient()
  ├── ExpedientStateMachine.transitionToValidated()
  │   (valida prerrequisitos, actualiza estado)
  └── ExpedientRepository.update()
      ↓
      Supabase.from('expedients').update().eq('id', id)
      ↓ (UPDATE SQL se ejecuta)
      Expediente actualizado, UI refleja cambio
```
...
### Estado UNDER_REVIEW
- ✅ Ver expediente
- ✅ Validar (transición a VALIDATED)
...
- ✅ Rol-based access control (OPERATOR vs SUPERVISOR)
...
### Prueba End-to-End Completa (10 min)
```
...
| Componentes React | 2 |
| Servicios | 3 |
| Endpoints Supabase | 5 (CRUD + queries) |
| Estados de expediente soportados | 7 |
| Acciones de usuario | 6 (Create, Review, Validate, Reject, Sign, Export) |
...
- [ ] `.env.local` configurado con Supabase credentials
...
1. **Supabase no conecta**
   ...
   - Ejecutar en console: `await supabase.auth.getSession()`
...
4. **Ctrl+E no funciona**
   - Revisar KeyDown listener en App.tsx
...
1. ✅ Separación clara entre servicios (Service → Repository → DB)
2. ✅ Manejo de estado con React hooks
3. ✅ Flujo de datos unidireccional
4. ✅ Transiciones de estado validadas
5. ✅ Integración de componentes sin breaking changes

### Lo que puede mejorarse en TIER 2+:
1. 🔄 Agregar real-time updates (Supabase subscriptions)
2. 🔄 Caché local (IndexedDB) para offline support
3. 🔄 Optimistic updates (actualizar UI antes de BD)
...
### Para Operadores
- ✅ Interface intuitiva para revisar infracciones
- ✅ Flujo de trabajo claro (DETECTED → VALIDATED → SIGNED → EXPORTED)
...
- ✅ Escalabilidad (serverless backend)
...
### Inmediato (Hoy)
1. Configurar .env.local con Supabase
2. Crear tabla en Supabase
3. Ejecutar `npm run dev`
4. Testing manual de E2E
...
**Status Final**: ✅ TIER 1 COMPLETADO Y LISTO PARA TESTING

**Siguiente Acción**: Ejecutar checklist de testing en TIER1_CHECKLIST.md
