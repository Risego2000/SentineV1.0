...
✅ `App.tsx` - Integración de vistas (Detección vs Expedientes)
...
### 2. Esquema Supabase - Tabla `expedients`
...
CREATE POLICY "Los usuarios pueden ver expedientes según su rol"
...
CREATE POLICY "Los operadores pueden actualizar sus expedientes"
...
3. Consulta a Supabase: `SELECT * FROM expedients WHERE state IN ('DETECTED', 'UNDER_REVIEW')`
...
│   ├── PDFExportService.ts          ✅ Nuevo (generación PDF)
│   └── SignatureService.ts          ✅ Existente (firmas digitales)
├── components/
│   └── ExpedientWorkflow.tsx        ✅ Nuevo (interfaz de flujo)
├── pages/
│   └── ExpedientListPage.tsx        ✅ Nuevo (lista + detalles)
├── utils/
│   └── supabaseClient.ts            ✅ Nuevo (inicialización Supabase)
├── domain/
│   ├── Expedient.ts                 ✅ Existente
│   ├── ExpedientStateMachine.ts     ✅ Existente
│   └── validation.ts                ✅ Existente
└── App.tsx                          ✅ Actualizado (cambio de vista)
```
...
### ExpedientRepository (Capa de Datos)
```typescript
// CRUD
...
const signed = await expedientService.signExpedient(expedient.id, {
  ...
  organization: 'Policía de Tráfico',
});
...
- [ ] Tabla `expedients` creada en Supabase con todas las columnas
...
- Conectar COCO-SSD a video en tiempo real
