...
import {
  ...
  syncGeometryToSupabase,
} from '../services/ipCameraService';
import { useFileWatcherSync } from '../hooks/useFileWatcherSync';
...
- Auditoría de estacionamientos prohibidos en zonas de carga.`);
  ...
  const [infractionLocation, _setInfractionLocation] = useState<string>(
    () => localStorage.getItem('sentinel_infraction_location') || ''
  );
  const [infractionLocations, setInfractionLocations] = useState<string[]>(() => {
    const saved = localStorage.getItem('sentinel_infraction_locations');
  ...
  const {
    ...
    isConnected: fileWatcherConnected,
    triggerSync,
  } = useFileWatcherSync(30000);

  useEffect(() => {
    if (fileWatcherConnected) {
      addLog('CORE', 'FileWatcher: Sincronización automática de evidencias activada.');
    }
  }, [fileWatcherConnected, addLog]);
  ...
  const setDirectives = useCallback(
    (d: string) => {
      ...
      // Update rules in Supabase (simplified as text directives for now,
      // but could be parsed into forensic_rules table)
    },
    [addLog]
  );
  ...
  const setInfractionLocation = useCallback(
    (location: string) => {
      _setInfractionLocation(location);
      localStorage.setItem('sentinel_infraction_location', location);
      addLog('INFO', `Lugar de infracción actualizado: ${location || 'Sin definir'}`);
    },
    [addLog]
  );

  const addInfractionLocation = useCallback((location: string) => {
    const normalized = location.trim();
    if (!normalized) return;
    setInfractionLocations((prev) => {
      const next = prev.includes(normalized) ? prev : [...prev, normalized];
      localStorage.setItem('sentinel_infraction_locations', JSON.stringify(next));
      return next;
    });
  }, []);

  const removeInfractionLocation = useCallback((index: number) => {
    setInfractionLocations((prev) => {
  ...
  const addInfraction = useCallback(
    (log: InfractionLog) => {
      ...
      const infractionWithViewer = { ...log, viewerId, infractionLocation };
      ...
      (async () => {
        try {
          const savedOutput = await ReportService.saveInfractionBundle(infractionWithViewer);
          const persistedInfraction = {
            ...infractionWithViewer,
            reportPath: savedOutput.path,
            generalImagePaths: savedOutput.generalImagePaths,
            detailImagePaths: savedOutput.detailImagePaths,
            videoPath: savedOutput.videoPath,
            storageFolder: savedOutput.folderPath,
          };

          await evidenceDB.saveInfraction(persistedInfraction);
          ...
          addLog('SUCCESS', `Infracción registrada en Excel diario: ${savedOutput.path}`);
          addLog('SUCCESS', `Carpeta de evidencias actualizada: ${savedOutput.folderPath}`);
        } catch (error) {
          addLog(
            'ERROR',
            `No se pudo guardar la salida de infracción: ${error instanceof Error ? error.message : String(error)}`
          );
        }
      })();
    },
    [addLog, setLogs, setStats, viewerId, infractionLocation]
  );
  ...
  const exportBatchReport = useCallback(async () => {
    ...
    addLog('INFO', 'Sincronizando infracciones detectadas con el Excel diario...');
    await ReportService.generateAndSaveBatchPdf(logs);
    addLog('SUCCESS', 'Excel diario actualizado.');
  }, [logs, addLog]);
  ...
  const finalizeVideoReport = useCallback(async () => {
    ...
    try {
      addLog('INFO', 'Esperando a que finalice la cola forense para consolidar el Excel diario...');
      await forensicQueue.waitForIdle();

      if (logsRef.current.length === 0) {
        addLog('WARN', 'El análisis finalizó sin infracciones confirmadas.');
        return;
      }

      const { path } = await ReportService.generateAndSaveBatchPdf(logsRef.current);
      addLog('SUCCESS', `Excel diario consolidado: ${path}`);
    } catch (error) {
      addLog(
        'ERROR',
        `No se pudo consolidar el Excel diario: ${error instanceof Error ? error.message : String(error)}`
      );
    } finally {
      reportGenerationRef.current = false;
    }
  ...
  useEffect(() => {
    ...
    // Sync all current geometries to Supabase
    geometry.forEach(syncGeometryToSupabase);
  }, [geometry, directives]);
  ...
  const value: SentinelContextType = useMemo(
    () => ({
      ...
      infractionLocations,
      setInfractionLocations,
      addInfractionLocation,
      removeInfractionLocation,
    }),
    [
      source,
      setSource,
