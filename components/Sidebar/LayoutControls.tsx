...
export const LayoutControls = () => {
  ...
  return (
    <div className="space-y-4 mb-4">
      ...
      >
        <LayoutGrid size={10} className="text-blue-500" /> VISUALIZACIÓN
      </h3>

      <div className="bg-slate-900/40 border border-white/10 rounded-[16px] p-3 space-y-3">
        <div className="flex items-center gap-1">
          <button
            onClick={() => setGridSize(1)}
            className={`flex-1 flex items-center justify-center py-2 rounded-[10px] border transition-all duration-300 ${
              gridSize === 1
        ...
        <div className="grid grid-cols-2 gap-2">
          <button
            ...
            className={`flex items-center justify-center gap-2 py-2 rounded-[10px] border transition-all duration-300 ${
              showDetections
                ? 'bg-blue-500/10 border-blue-500 text-blue-500 shadow-[0_0_20px_rgba(59,130,246,0.2)]'
                : 'bg-black/20 border-white/5 text-slate-500 hover:bg-white/[0.04] hover:text-slate-300'
            }`}
          ...
          >
            {showDetections ? <ScanLine size={12} /> : <EyeOff size={12} />}
          ...
          >
            {showROIs ? <Map size={12} /> : <EyeOff size={12} />}
            <span className="text-[8px] font-black uppercase tracking-wider">
