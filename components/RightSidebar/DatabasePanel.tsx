...
import { RefreshCw, ChevronDown, AlertTriangle, Loader, Image } from 'lucide-react';
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-2">
          {!isSupabaseConfigured ? (
            ...
            data.slice(0, 10).map((row, idx) => (
              <div
                key={idx}
                className="p-2 bg-white/[0.02] border border-white/5 rounded text-[8px] space-y-1"
              >
                  ...
                  {Object.entries(row)
                    ...
                    .map(([key, val]) => (
                      <div key={key} className="truncate">
                        ...
                        <span className="text-slate-400">{String(val).slice(0, 30)}</span>
                      </div>
                    ))}
                </div>
              </div>
